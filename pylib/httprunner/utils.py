...
def deep_update_dict(origin_dict, override_dict):
    ...
    #zhengchun, 加入对list的处理, 20180608
    if isinstance(override_dict, (list, tuple)):
        for val in override_dict:
            if isinstance(val, dict):
                tmp = deep_update_dict({}, val)
                origin_dict.append(tmp)
...
def is_function(tup):
    ...
    return isinstance(item, (types.FunctionType, types.BuiltinFunctionType))
