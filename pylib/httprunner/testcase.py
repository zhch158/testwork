...
variable_regexp = r"\$(\{)?([\w_]+)(?(1)\})" #接受2种变量，$var or ${var}
# variable_regexp = r"\$([\w_]+)"
# variable_regexp = r'\$([\w_]+)|\{([\w_]+)\}'
...
def extract_variables(content):
    ...
    try:
        ...
        var_list=list()
        for var_name in re.finditer(variable_regexp, content):
            var_list.append(var_name.group(2))
        return var_list
    except TypeError:
        return []
        ...
        for item in FileUtils.load_file(file_path):
            ...
            if key == "config":
                ...
                if "runner" in test_block:
                    ...
                    inx=s.rfind(".")
                    if(inx<=0):
                        raise exception.ParamsError("runner format error[%s]" %(s))
                    module_name=s[0:inx]
                    cls_name=s[inx+1:]
...
def parse_parameters(parameters, testset_path=None, parser_handle=None):
...
class TestcaseParser(object):
    ...
    def _eval_content_variables(self, content):
        ...
        for variable_name in variables_list:
            ...
            #zhengchun, 20180608
            var_content = "${" + variable_name + "}"
            if var_content == content or "${}".format(variable_name) == content:
                # content is a variable
                content = variable_value
            else:
                # content contains one or several variables
                if(content.find(var_content)>=0):
                    content = content.replace( var_content, str(variable_value), 1)
                else:
                    content = content.replace( "${}".format(variable_name), str(variable_value), 1)
